...
export default function ListeningLessonsPage() {
  ...
  // Fetch speaking lessons from Contentful
  useEffect(() => {
    const fetchLessons = async () => {
      ...
      try {
        const response = await client.getEntries({
          ...
          "fields.typeOfLesson": "Listening"
        });

        const listeningLessons: Lesson[] = response.items.map((item: any) => {
          let audioUrl = "";

          // Handle audio field
          if (item.fields.audio && item.fields.audio.fields && item.fields.audio.fields.file) {
            const audioFileUrl = item.fields.audio.fields.file.url;
            audioUrl = audioFileUrl.startsWith("http") ? audioFileUrl : `https:${audioFileUrl}`;
          }

          return {
            id: item.sys.id,
            title: item.fields.lessonTitle || "Unnamed Speaking Lesson",
            language: item.fields.language || "Kiswahili",
            level: item.fields.level || "Beginner",
            typeOfLesson: "Listening",
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        ...
        <View style={styles.userInfo}>
          ...
          <Text style={styles.subText}>Listening Lessons</Text>
      ...
      {loading ? (
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>Loading listening lessons...</Text>
        </View>
      ) : (
        <ScrollView
        ...
        >
          <View style={styles.section}>
            <View style={[styles.sectionHeader, { backgroundColor: "#FFB6C1" }]}>
              <Image 
                source={require("../../assets/images/speaking.png")} 
                style={styles.sectionIcon} 
              />
              <View style={styles.sectionTitleContainer}>
                <Text style={styles.sectionTitle}>Listening Lessons</Text>
                <Text style={styles.sectionDescription}>Practice listening and comprehension</Text>
              </View>
            </View>
            ...
            {lessons.length > 0 ? (
              ...
              <View style={styles.noLessonsContainer}>
                <Text style={styles.noLessonsText}>No listening lessons available yet</Text>
